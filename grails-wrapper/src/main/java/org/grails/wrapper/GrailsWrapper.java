...
public class GrailsWrapper {
    ...
    public static void main(final String[] args) throws Exception{
        final String wrapperLaunchClasspath = System.getProperty("grails.wrapper.launch.classpath");
        final ResourceBundle gradleBundle = ResourceBundle.getBundle("gradle");
        final ResourceBundle wrapperBundle = ResourceBundle.getBundle("grails-wrapper");
        final String grailsVersion = gradleBundle.getString("grailsVersion");
        String distUrl = wrapperBundle.getString("wrapper.dist.url");
        if (distUrl == null) {
            distUrl = "https://github.com/grails/grails-core/releases/download/";
        }
        if (!distUrl.endsWith("/")) {
            distUrl += "/";
        }
        ...
        final String[] wrapperLaunchClasspathPieces = wrapperLaunchClasspath.split(":");
        for(String piece : wrapperLaunchClasspathPieces) {
            final String path = piece.replace("@grails_home@", grailsHome.getAbsolutePath());
            final File f = new File(path);
            urlList.add(f.toURI().toURL());
        }
        final URLClassLoader urlClassLoader = new URLClassLoader(urlList.toArray(new URL[urlList.size()]));

        final Class<?> loadClass = urlClassLoader.loadClass("org.grails.cli.GrailsCli");
    ...
    private static File configureGrailsInstallation(String distUrl,
        ...
        final String src = distUrl + "v" + grailsVersion + "/grails-" + grailsVersion + ".zip";
