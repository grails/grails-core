 ...
 */
@CompileStatic
class HibernateChangedSequenceChangeGenerator extends ChangedSequenceChangeGenerator {
    ...
    int getPriority(Class<? extends DatabaseObject> objectType, Database database) {
        return Sequence.isAssignableFrom(objectType) ? PRIORITY_ADDITIONAL : PRIORITY_NONE
    }
    ...
    private void filterIrrelevantDifferences(ObjectDifferences differences, Database refDb, Database compDb) {
        ...
        for (Difference diff : differences.getDifferences()) {
            if (isIgnoredField(diff) || isAdvancedIgnoredDifference(diff, refDb, compDb)) {
                fieldsToRemove.add(diff.getField())
            }
        }
