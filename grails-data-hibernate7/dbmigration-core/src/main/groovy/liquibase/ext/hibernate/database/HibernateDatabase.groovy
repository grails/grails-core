 ...
 *
 * <p>Compiled as a {@link POJO} (no {@code GroovyObject} implementation) because the public
 * {@link #getProperty(String)} API method would otherwise be treated by Groovy's runtime as the
 * dynamic property-access hook and hijack {@code database.someProperty} for every dynamic caller.
 * Subclasses must carry the same annotation.
 */
@CompileStatic
@POJO
abstract class HibernateDatabase extends AbstractJdbcDatabase {
    ...
    protected HibernateConnection getHibernateConnection() {
        ...
        if (originalConnection instanceof JdbcConnection) {
            Connection underlyingConnection = ((JdbcConnection) originalConnection).getUnderlyingConnection()
    ...
    protected final Metadata buildMetadata() throws DatabaseException {
        ...
        if (!path.contains('/')) {
            try {
                ...
                if (CustomMetadataFactory.isAssignableFrom(clazz)) {
                    try {
                        return ((CustomMetadataFactory) clazz.getDeclaredConstructor().newInstance())
    ...
    protected Metadata buildMetadataFromPath() throws DatabaseException {
        ...
        Thread t = new Thread({ -> result.set(metadataBuilder.build()) } as Runnable)
        t.setContextClassLoader(Thread.currentThread().getContextClassLoader())
        t.setUncaughtExceptionHandler({ Thread thread, Throwable e -> thrownException.set(e) } as Thread.UncaughtExceptionHandler)
        t.start()
        try {
            t.join()
            ...
            if (namingStrategy != null) {
                switch (namingStrategy) {
                    ...
                    case 'jpa':
                        ...
                        break
                    case 'legacy-hbm':
                        builder.applyImplicitNamingStrategy(ImplicitNamingStrategyLegacyHbmImpl.INSTANCE)
                        break
                    case 'legacy-jpa':
                        builder.applyImplicitNamingStrategy(ImplicitNamingStrategyLegacyJpaImpl.INSTANCE)
                        break
                    case 'component-path':
