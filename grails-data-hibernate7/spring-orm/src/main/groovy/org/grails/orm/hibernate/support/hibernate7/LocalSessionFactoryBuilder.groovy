...
class LocalSessionFactoryBuilder extends Configuration {
    ...
    private static final TypeFilter[] DEFAULT_ENTITY_TYPE_FILTERS = [
        ...
        new AnnotationTypeFilter(MappedSuperclass, false)] as TypeFilter[]

    private static final TypeFilter CONVERTER_TYPE_FILTER = new AnnotationTypeFilter(Converter, false)

    private static final String IGNORE_CLASSFORMAT_PROPERTY_NAME = 'spring.classformat.ignore'

    private static final boolean SHOULD_IGNORE_CLASS_FORMAT_EXCEPTION =
    ...
    LocalSessionFactoryBuilder(
        ...
        getProperties().put(AvailableSettings.CURRENT_SESSION_CONTEXT_CLASS, SpringSessionContext.name)
        if (dataSource != null) {
    ...
    LocalSessionFactoryBuilder setJtaTransactionManager(Object jtaTransactionManager) {
        ...
        if (jtaTransactionManager instanceof JtaTransactionManager) {
            JtaTransactionManager springJtaTm = (JtaTransactionManager) jtaTransactionManager
     ...
     */
    LocalSessionFactoryBuilder setMultiTenantConnectionProvider(MultiTenantConnectionProvider multiTenantConnectionProvider) {
     ...
     */
    @Override
    Configuration setCurrentTenantIdentifierResolver(CurrentTenantIdentifierResolver<?> currentTenantIdentifierResolver) {
            ...
            for (String pkg : packagesToScan) {
                ...
                for (Resource resource : resources) {
                    try {
                        ...
                        if (!SHOULD_IGNORE_CLASS_FORMAT_EXCEPTION) {
     ...
     */
    SessionFactory buildSessionFactory(AsyncTaskExecutor bootstrapExecutor) {
        ...
        return (SessionFactory) Proxy.newProxyInstance(this.resourcePatternResolver.getClassLoader(),
                [SessionFactoryImplementor, InfrastructureProxy] as Class<?>[],
    ...
    private class BootstrapSessionFactoryInvocationHandler implements InvocationHandler {
        ...
        BootstrapSessionFactoryInvocationHandler(AsyncTaskExecutor bootstrapExecutor) {
            this.sessionFactoryFuture = bootstrapExecutor.submit(
                    { -> LocalSessionFactoryBuilder.this.buildSessionFactory() } as Callable<SessionFactory>)
        }
