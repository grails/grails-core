...
abstract class SessionFactoryUtils {
    ...
    @PackageScope
    static final Log LOGGER = LogFactory.getLog(SessionFactoryUtils)
    ...
    static DataSource getDataSource(SessionFactory sessionFactory) {
        ...
        if (sessionFactory instanceof SessionFactoryImplementor) {
            ...
            try {
                ConnectionProvider cp = sfi.getServiceRegistry().getService(ConnectionProvider)
                if (cp != null) {
                    return cp.unwrap(DataSource)
                }
