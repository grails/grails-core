 ...
 */
package org.grails.dependency.resolution.ivy.config
...
import org.grails.dependency.resolution.ivy.GrailsPluginsDirectoryResolver
import org.grails.dependency.resolution.ivy.GrailsRepoResolver
import org.grails.dependency.resolution.ivy.SnapshotAwareM2Resolver
...
class RepositoriesConfigurer extends AbstractDependencyManagementConfigurer {
    ...
    void grailsCentral() {
        if (!context.offline && isResolverNotAlreadyDefined('grailsCentral')) {
            mavenRepo(root:"https://repo.grails.org/grails/plugins", name:"grailsCentral")
        }
    }
        ...
        if (isResolverNotAlreadyDefined('mavenLocal')) {
            ...
            if (!repositoryPath) {
                ...
                if (mavenSettingsFile.exists()) {
                    def settingsXml = SpringIOUtils.createXmlSlurper().parse(mavenSettingsFile)
