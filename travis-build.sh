...
if [[ $TRAVIS_PULL_REQUEST == 'false' && $EXIT_STATUS -eq 0 ]]; then
    ...
    if [[ $TRAVIS_TAG =~ ^v[[:digit:]] ]]; then
        ./gradlew -Psigning.keyId="$SIGNING_KEY" -Psigning.password="$SIGNING_PASSPHRASE" -PsigningFile="${TRAVIS_BUILD_DIR}/local.secring.gpg" uploadArchives publish || EXIT_STATUS=$?
