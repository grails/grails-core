...
class GroovyPageScanner implements Tokens {
    ...
    int nextToken() {
        for (;;) {
            ...
            else if (!levelTokens.empty() && (c == ')' || c == '}' || c == ']') && state != JSCRIPT && levelTokens.peek().equals(c)) {
                levelTokens.pop();
                continue;
            } else if (begin1 > 0 && (c == '{' || c == '(' || c == '[') && state != JSCRIPT) {
                if (c == '{') {
                    levelTokens.push('}');
                    continue;
                } else if (c == '(') {
            ...
            switch (state) {
                ...
                case GDIRECT:
                    ...
                    break;
                case GSCRIPT:
                    if (c == '}' && isStartScriptletBlock(c1) && !str1 && !str2) {
                        return found(HTML, 2);
                    }
                    break;
                case GDECLAR:
                    if (c == '}' && (c1 == '!' || isStartScriptletBlock(c1)) && !str1 && !str2) {
                        return found(HTML, 2);
                    }
                    break;
            }
