 ...
 */
@CompileStatic
abstract class AbstractMongoObectEntityPersister<T> extends NativeEntryEntityPersister<T, Object> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(AbstractMongoObectEntityPersister)
    protected boolean hasNumericalIdentifier = false
    protected boolean hasStringIdentifier = false

    AbstractMongoObectEntityPersister(MappingContext mappingContext, PersistentEntity entity, Session session, ApplicationEventPublisher publisher) {
        super(mappingContext, entity, session, publisher)
        if (!(entity instanceof EmbeddedPersistentEntity)) {
            PersistentProperty identity = entity.getIdentity()
            if (identity != null) {
                hasNumericalIdentifier = Long.isAssignableFrom(identity.getType())
                hasStringIdentifier = String.isAssignableFrom(identity.getType())
            }
        }
    }

    static String createInstanceCacheEntryKey(Object instance) {
    ...
    @Override
    boolean isDirty(Object instance, Object entry) {
    ...
    protected List<Object> retrieveAllEntities(PersistentEntity persistentEntity,
        ...
        for (Object o : query.list()) {
            Object result = o
            if (isEmbeddedEntry(result)) {
                final ValueRetrievalStrategy<T> valueRetrievalStrategy = getValueRetrievalStrategy()
                final Object id = valueRetrievalStrategy.getValue((T) result, MONGO_ID_FIELD)
                result = createObjectFromNativeEntry(getPersistentEntity(), (Serializable) id, (T) result)
            }
            resultMap.put(getObjectIdentifier(result), result)
        }
    ...
    protected class MongoAssociationIndexer implements AssociationIndexer {
        ...
        MongoAssociationIndexer(T nativeEntry, Association association, MongoSession session) {
            this.nativeEntry = nativeEntry
            this.association = association
            this.session = session
            this.reference = AbstractMongoObectEntityPersister.this.isReference(association)
        }
        ...
        void preIndex(final Object primaryKey, final List foreignKeys) {
        ...
        void index(final Object primaryKey, final List foreignKeys) {
