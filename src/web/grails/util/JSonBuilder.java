...
public class JSonBuilder extends BuilderSupport {
	...
	protected Object createNode(Object name) {
		...
		try {
			if( start ){
				...
				if( getCurrent() == null && stack.peek() == Mode.OBJECT) throw new IllegalArgumentException( JSON_BUILDER + "only call to [element { }] is allowed when creating array");
	...
	protected Object createNode(Object key, Object value) {
		if( getCurrent() == null && stack.peek().equals(Mode.OBJECT)) throw new IllegalArgumentException( JSON_BUILDER + "only call to [element { }] is allowed when creating array");
		try {
			int retVal = 0;
