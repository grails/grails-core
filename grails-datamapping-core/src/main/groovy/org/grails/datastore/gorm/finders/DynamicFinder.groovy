...
abstract class DynamicFinder extends AbstractFinder implements QueryBuildingFinder {
    ...
    private static final String[] DEFAULT_OPERATORS = [OPERATOR_AND, OPERATOR_OR] as String[]
    ...
    static {
        ...
        try {
            Class[] classes = [
                Equal, NotEqual, NotInList, InList, InRange, Between, Like, Ilike, Rlike,
                GreaterThanEquals, LessThanEquals, GreaterThan,
                LessThan, IsNull, IsNotNull, IsEmpty,
                IsEmpty, IsNotEmpty] as Class[]
            Class[] constructorParamTypes = [Class, String] as Class[]
            for (Class c : classes) {
     ...
     */
    static void registerNewMethodExpression(Class methodExpression) {
        try {
     ...
     */
    static MatchSpec buildMatchSpec(String prefix, String methodName, int parameterCount) {
    ...
    Object invoke(final Class clazz, String methodName, Closure additionalCriteria, Object[] arguments) {
    ...
    Object invoke(final Class clazz, String methodName, DetachedCriteria detachedCriteria, Object[] arguments) {
    ...
    DynamicFinderInvocation createFinderInvocation(Class clazz, String methodName,
        ...
        if (groupCount == 6) {
            ...
            booleanExpression.setArguments([arg] as Object[])
    ...
    Object invoke(final Class clazz, String methodName, Object[] arguments) {
     ...
     */
    static void populateArgumentsForCriteria(BuildableCriteria query, Map argMap) {
        if (argMap == null) {
            return
        }
    ...
    static void populateArgumentsForCriteria(Class<? extends Object> targetClass, Query query, Map argMap) {
        if (argMap == null) {
            return
        }
        ...
        if (argMap.containsKey(ARGUMENT_MAX)) {
            maxParam = conversionService.convert(argMap.get(ARGUMENT_MAX), Integer)
        }
        if (argMap.containsKey(ARGUMENT_OFFSET)) {
            offsetParam = conversionService.convert(argMap.get(ARGUMENT_OFFSET), Integer)
        }
        String orderParam = (String) argMap.get(ARGUMENT_ORDER)
     ...
     */
    static void applySortForMap(Query query, Map sortMap, boolean ignoreCase) {
        for (Object key : sortMap.keySet()) {
            Object value = sortMap.get(key)
            String sort = key.toString()
     ...
     */
    static FetchType getFetchMode(Object object) {
    ...
    private static MethodExpression findMethodExpressionInternal(final Class clazz, String expression) {
        ...
        Class methodExpressionClass = Equal
    ...
    private static String calcPropertyName(String queryParameter, String clause) {
        String propName
        if (clause != null && !clause.equals(Equal.getSimpleName())) {
    ...
    protected Query.Junction getJunction(DynamicFinderInvocation invocation) {
        def criteria = invocation.getExpressions().stream().map(MethodExpression::createCriterion).collect(Collectors.toList())
    ...
    Query buildQuery(DynamicFinderInvocation invocation, Session session) {
        ...
        def query = session.createQuery(clazz)
