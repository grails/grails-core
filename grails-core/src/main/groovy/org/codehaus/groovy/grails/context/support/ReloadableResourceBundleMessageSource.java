...
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
     ...
     * <li>A positive number will cache the list of matching properties files
     * for the given number of seconds. This is essentially the interval between
     * refresh checks.
            ...
            for (String basename : this.basenames) {
                ...
                for (Pair<String, Resource> filenameAndResource : filenamesAndResources) {
                    if (filenameAndResource.getbValue() != null) {
                        PropertiesHolder propHolder = getProperties(filenameAndResource.getaValue(),
                                filenameAndResource.getbValue());
                        String result = propHolder.getProperty(code);
                        if (result != null) {
                            return result;
                        }
    ...
    protected List<Pair<String, Resource>> calculateAllFilenames(final String basename, final Locale locale) {
        ...
        return CacheEntry.getValue(cachedFilenames, cacheKey, cacheMillis,
                new Callable<List<Pair<String, Resource>>>() {
                    @Override
                        ...
                        if (fallbackToSystemLocale && !locale.equals(Locale.getDefault())) {
                            ...
                            for (String fallbackFilename : fallbackFilenames) {
                                if (!filenames.contains(fallbackFilename)) {
                                    // Entry for fallback locale that isn't
                                    // already in filenames list.
                                    filenames.add(fallbackFilename);
                                }
                            }
                        }
                        filenames.add(basename);
                        List<Pair<String, Resource>> filenamesAndResources = new ArrayList<Pair<String, Resource>>(
                                filenames.size());
    ...
    public void clearCacheIncludingAncestors() {
        ...
        if (getParentMessageSource() instanceof ReloadableResourceBundleMessageSource) {
            ...
            ((org.springframework.context.support.ReloadableResourceBundleMessageSource)getParentMessageSource())
